...
<template>
  ...
  >
    <div>页面内容</div>
    <template #subheader>
      <div style="padding: 12px 0; color: #475569;">这里可以承载模块级 tab、筛选摘要或二级操作条。</div>
    </template>
  </PlatformLayout>
</template>`,
...
const componentConfig: Record<string, ComponentDocConfig> = {
  ...
  'platform-layout': {
    ...
    slots: [
      slot('brand', '-', '自定义左侧品牌区。'),
      slot('header-actions', '-', '顶部右侧扩展操作区的插槽。'),
      slot('subheader', '-', '顶部下方的二级承载区，适合承载 portal-shell 的持久化 tabs、筛选条或二级操作栏。'),
      slot('default', '-', '主内容区。')
    ],
    notes: [
      'showAside=false 时适合门户工作台页，内容区会占满主区域。',
      'portal-shell 推荐把持久化模块 tabs 放在 subheader 插槽，由 host 统一管理 tabs 的打开、关闭与恢复。',
