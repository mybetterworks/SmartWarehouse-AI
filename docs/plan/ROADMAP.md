...
- Gateway 接入 Nacos Discovery，`/api/sys/**` 默认通过 `lb://sys-service` 进行服务发现和负载转发。
- sys-service 用户登录、退出、刷新 Token。
- sys-service 接入 Nacos Discovery，本地 Nacos 默认使用 `127.0.0.1:18848`，容器内使用 `nacos:8848`。
- 用户、角色、菜单、部门、岗位、字典管理。
- 登录风控：连续失败 3 次后启用随机拼图验证码。
- 操作日志、登录日志、风控记录。
- 数据权限：仓库管理员只能查看自己负责的仓库，不同角色看到不同菜单。
- 后端代码按业务模块拆分 Controller，禁止把认证、用户、角色、菜单、组织、字典、日志和风控全部堆在一个管理控制器中。
...
- `portal-shell` 登录后根据后端授权菜单和模块列表展示入口，所有前端业务模块必须在门户统一入口 `http://localhost:5174/` 下通过路由访问，例如 `/sys/users`、`/wms/**`、`/mes/**`、`/ai/**`；不得从门户跳到各自独立 dev server 端口作为集成入口。
- `portal-shell` 使用 `vite-plugin-federation` 作为微前端 host，运行时根据 `sys_frontend_module.remote_name`、`remote_entry`、`exposed_module` 加载 `sys-web`、`wms-web`、`mes-web`、`ai-web` remote。
- 乙方前端模块发布新版本时，只更新自己的静态制品和模块注册信息，不需要重新构建或发布 `portal-shell`。
- 如果某个 remote 加载失败或超时，`portal-shell` 只显示该模块降级页，门户、系统管理和其他 remote 不受影响。
- `portal-shell` 与 `sys-web` 均支持右上角个人信息和修改密码入口，其中改密接口由 `sys-service` 提供。
- `sys-web`、`wms-web`、`mes-web`、`ai-web` 均按独立项目开发、独立构建、独立部署，门户集成时暴露 `./RemoteApp` 给 `portal-shell` 运行时加载；不再通过 `redirect` 参数跳转到 `localhost:5175/apps/sys/`，也不再把乙方应用作为构建期依赖打进门户。
- 前端代码按业务页面拆分 views，禁止把用户、角色、菜单、部门、字典、日志、风控等所有页面写在一个 `App.vue` 中。
- `wms-web` 仓储业务页面。
- `mes-web` 生产业务页面。
- `ai-web` AI 助手页面。
...
| 实时推送 | WebSocket |
| 前端 | Vue、Vite、TypeScript、Element Plus、Pinia、Vue Router、vite-plugin-federation |
...
14. 开发、联调、测试发布或排查问题时，AI 可以按需要操作本地 Docker Desktop，包括拉取镜像、构建镜像、启动或重启容器、执行容器内初始化/验证脚本和读取容器日志；但删除卷、清空数据、系统级 prune 等可能丢失数据的操作必须先获得用户确认。
15. 自动测试必须使用与人工测试一致的本地明确配置，连接 Docker 中的真实数据库和中间件；自动测试通过后，用户应能按同一套默认配置直接启动服务和前端页面，不需要额外修改端口、账号或连接串。
```
...
15. AI 可以按需求操作本地 Docker Desktop 完成开发验证，包括镜像拉取、镜像构建、Docker Compose 启停、容器内脚本执行、健康检查和日志排查；涉及删除数据卷、清空数据库、删除镜像或 prune 类危险操作时必须先确认。
